from django.db import models
from django.contrib.auth.models import AbstractUser, Group
from courses.models import Course
from courses.models import Subject

class User(AbstractUser):
  USER_CHOICES = (
    ('S', 'STUDENT'),
    ('I', 'INSTRUCTOR'),
  )
  GENDER_CHOICES = (
    ('M', 'Male'),
    ('F', 'Female'),
  )

  user_role     = models.CharField(max_length=15, choices=USER_CHOICES)
  gender        = models.CharField(max_length=10, choices=GENDER_CHOICES, default='M')
  profile_image = models.ImageField(upload_to='files/profile_images/', default='files/profile_images/default.png')

  def save(self, *args, **kwargs):
    created = self.id is None
    super(User, self).save(*args, **kwargs)
    if created and self.user_role == 'I':
      self.groups.add(Group.objects.get(pk=1))

  @property
  def full_name(self):
    return f"{self.first_name} {self.last_name}"

class InstructorProfile(models.Model):
  user   = models.OneToOneField(User, related_name='instructor_profile', on_delete=models.CASCADE)
  major_subjects    = models.ManyToManyField(Subject, blank=True)
  place_of_birth    = models.CharField(max_length=30, null=True, blank=True)
  about             = models.TextField(null=True, blank=True)
  achievements      = models.TextField(null=True, blank=True)
  objectives        = models.TextField(null=True, blank=True)
  facebook_profile  = models.URLField(null=True, blank=True)
  twitter_profile   = models.URLField(null=True, blank=True)
  github_profile    = models.URLField(null=True, blank=True)
  instagram_profile = models.URLField(null=True, blank=True)

  def enrolled_students(self):
    courses = self.courses.all()
    enrolled_students = []
    for course in courses:
      enrolled_students += course.students.all()
    return len(set(enrolled_students))

  def __str__(self):
    return f"{self.user.username} added as Instructor"


# <   1 - many
# >  many - many
# -  1 - 1

class StudentProfile(models.Model):
  user          = models.OneToOneField(User, 
                                      related_name='student_profile', 
                                      on_delete=models.CASCADE)
  bio           = models.TextField(null=True, blank=True)

  def get_unanswered_questions(self, quiz):
    answered_question = self.quiz_answers.filter(answer__question__quiz=quiz).values_list('answer__question__pk', flat=True)
    questions = quiz.questions.exclude(pk__in=answered_question)
    return questions

  def __str__(self):
    return f"{self.user.username} added as Student"